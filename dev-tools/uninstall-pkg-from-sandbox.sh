...
echoStepMsg "Attempt to uninstall package $PACKAGE_VERSION_ID from the org aliased as $SUBSCRIBER_ORG_ALIAS"
echo \
"Executing sfdx force:package:uninstall \\
                -i $PACKAGE_VERSION_ID \\
                -w 15 \\
                -u $SUBSCRIBER_ORG_ALIAS \\
                --loglevel error\n"
...
if [ $? -ne 0 ]; then
  echoErrorMsg "Package removal from $SUBSCRIBER_ORG_ALIAS was not successful."
fi

#### 2. List all of the packages installed in the target subscriber org.
echoStepMsg "List all packages that remain in $SUBSCRIBER_ORG_ALIAS"
echo "List of packages currently installed in $SUBSCRIBER_ORG_ALIAS.\n"
